    ...
    def flush(self):
        if self._file is not None:
            try:
                ...
                # don't flush the output or it stays a long while the
                # instrument has been unplugged of the serial line.
...
class Omega(taurus.Logger):
    ...
    # #communications ---
    def open(self):
        if self._thread is not None and self._thread.isAlive() and \
        ...
        if self._serial is not None and self._serial.open():
            self._joinerEvent.clear()
            self._thread.start()
            return True
        else:
    ...
    def _sendAndReceive(self, cmd, blocking=True, postprocess=True):
        ...
        if not self._flush():
            self.pushStatusCallback(taurus.Logger.Error, "Flush exception, "
                                    "check communications")
        if not self.P(blocking):
            self.debug("abort _sendAndReceive()")
            return float('NaN')
        if self._serial is None:
            self.pushStatusCallback(taurus.Logger.Error, "Non opened "
                                    "instrument cannot do send&receive")
    ...
    def reconnect(self):
        ...
        try:
            ...
            #       "recoconnections)" % (RECONNECTDELAY,
            #                             self._failedReconnections)
            msg = "Too many failed readings, review connection and call Init()"
            self.pushStatusCallback(taurus.Logger.Error, msg)
            self.warning(msg)
            if self._serial is None:
                self.pushStatusCallback(taurus.Logger.Error, "Non existing "
                                        "instrument cannot be reconnected.")
                return
            self._serial.close()
    ...
    def _postprocessAnswer(self, cmd, answer):
        ...
        try:
            ...
            self.pushStatusCallback(taurus.Logger.Warning, "Answer %r cannot "
                                    "be isolated." % (repr(answer)))
            return float('NaN')
        cmdLongName = self._getKeyFromItemInDict(self.commands, command)
        if not cmdLongName:
            cmdLongName = self._getKeyFromItemInDict(self.commands,
                                                     command[2:])
        ...
        if cmdLongName in self._floatCmds:
            ...
            return float(answer)
        elif cmdLongName in self._hexlistCmds:
    ...
    def subscribe(self, command, callback=None):
        '''
            With this method a class that has an instance of this one, will
            have a subscription identifier in the return value to manage
            unsubscriptions.
        ...
        if command not in self._answers:
    ...
    def unsubscribe(self, id):
        ...
        if not self._identifiers.count(id):
            raise LookupError("Not recognised %d identifier" % (id))
        if len(self._subscribers.keys()) > 0:
            self._threadLoopCondition.acquire()
            for cmd in self._subscribers.keys():
                if id in self._subscribers[cmd]:
            ...
            self._threadLoopCondition.release()
        # FIXME: if there are no more subscriber,
        #        may signal the thread to rest
        ...
        while not self._joinerEvent.isSet():
            ...
            try:
                ...
                for command in self._answers:
                    ...
                    try:
                        bar = self.commands[command]
                        self._answers[command] = self._sendAndReceive(bar)
                    except ValueError as e:
                        ...
                        if self._failedReconnections >= RECONNECTLIMIT:
                            ...
                            try:
                                bar = self.commands[command]
                                foo = self._sendAndReceive(bar)
                                self._answers[command] = foo
                            except:
                ...
                for command in self._subscribers:
                    if command in self._answers:
                        if self._answers[command] is not None:
                ...
                traceback.print_exc()
                self.pushStatusCallback(taurus.Logger.Error, "Communication "
                                        "cut. Please Init() the device.")
            self._threadLoopCondition.release()
        ...
        for id in self._subscribers[command]:
            ...
            if cb is not None:
                try:
                    ...
                    self.error("Callback exception for id %d: %s" % (id, e))
    # done Subscription ---
    #####
...
def parallelWriteTest(omega):
    '''
        We may speed up multiple readings by doing write operation with
        multiple requests. But the instrument looks that doesn't support it.
    '''
    ...
    if answer.count("\r") != 0:
        ...
        for i, cmd in enumerate([UNFILTERED, PEAK, VALLEY, FILTERED]):
            value = omega._postprocessAnswer(omega.commands[cmd], alist[i])
            if cmd == UNFILTERED:
                unfiltered = value
            elif cmd == PEAK:
                peak = value
